...
var modalidad = {
    calendario: function (req, res) {
        ...
        pg.connect(connectionString.DAC, function (err, client, done) {
            ...
            query = "select min(a.cantidad) as minCant,MAX(a.cantidad)as maxCant,MIN(EXTRACT(YEAR FROM a.fecha)) as minFecha, (MAX(EXTRACT(YEAR FROM a.fecha))+1) as maxFecha from (select t.id_tie_fecha as fecha,id_tie_anio as anio, COUNT(*) as cantidad from dw.ft_modalidad mod left outer join dw.lk_tiempo t on t.id_fecha=id_fecha_hecho left outer join dw.lk_seccional secc on secc.id_secc_seccional=mod.id_sec_seccional where  1=1";

            if (typeof req.query.val != 'undefined' && req.query.val.length > 0) {
                query = query.concat(" and id_mod_modalidad in (").concat(req.query.val).concat(")");
            }
            else {
    ...
    datos: function (req, res) {
        ...
        pg.connect(connectionString.DAC, function (err, client, done) {
            ...
            {
                query = " FROM dw.ft_modalidad carg left outer join dw.lk_seccional secc on secc.id_secc_seccional=carg.id_sec_seccional where 1=1  and id_fecha_hecho >= ".concat(req.query.fecha_d).concat(" and id_fecha_hecho <= ").concat(req.query.fecha_h)
            }
            else {
    ...
    barrioCloroDatos: function (req, res) {
        ...
        pg.connect(connectionString.DAC, function (err, client, done) {
            //    var query_res = client.query("SET SEARCH_PATH TO ".concat(connectionString.DAC_schema));
            query = "select cod_bahra as barrio,count(*) as cantidad FROM dw.ft_modalidad mod  left outer join dw.lk_seccional secc on secc.id_secc_seccional=mod.id_sec_seccional where 1=1"
            if (typeof req.query.fecha_d != 'undefined' && typeof req.query.fecha_h != 'undefined') {
            ...
            if (typeof req.query != 'undefined' && req.query.sec.length > 0) {
                query = query.concat(" and id_sec_seccional in (").concat(req.query.sec).concat(") ");
            }
            if (typeof req.query.calle != 'undefined' && req.query.calle.length > 0) {
                query = query.concat(" and upper(lugar_hecho) like ('%").concat(req.query.calle.toUpperCase()).concat("%') ");
            }
            query = query.concat(" and cod_bahra is not null and cod_bahra != '0' ");
            ...
            query_res.on('row', function (row) {
                for (var i = 0; i < results.length; i++) {
                    console.log(row.barrio);
                    console.log(results[i].bahra);
    ...
    tags: function (req, res) {
        ...
        pg.connect(connectionString.DAC, function (err, client, done) {
            ...
            if (typeof req.query.val != 'undefined') {
                consulta = " and mod.id_mod_modalidad in (".concat(req.query.val).concat(")");
            }
            else {
                return res.status(500).send('ERRROOOOOOOR con el delito');
            }
    ...
    total: function (req, res) {
        ...
        pg.connect(connectionString.DAC, function (err, client, done) {
            var query_res = client.query("SET SEARCH_PATH TO ".concat(connectionString.DAC_schema));
            query = "select * from ListarModalidadN2(";
    ...
    hora: function (req, res) {
        ...
        pg.connect(connectionString.DAC, function (err, client, done) {
            var query_res = client.query("SET SEARCH_PATH TO ".concat(connectionString.DAC_schema));
            query = "select * from horariomodalidadn2(";
            ...
            if (typeof req.query.val != 'undefined') {
                consulta = " and id_mod_modalidad in (".concat(req.query.val).concat(")");
            }
            else {
                return res.status(500).send('ERRROOOOOOOR con la modalidad');
            }
    ...
    puntos: function (req, res) {
        ...
        pg.connect(connectionString.DAC, function (err, client, done) {
            ...
            if (typeof req.query.val != 'undefined') {
                consulta = consulta.concat(" and mod.id_mod_modalidad in (").concat(req.query.val).concat(")");
            }
            else {
                return res.status(500).send('ERRROOOOOOOR con el delito');
            }
    ...
    check: function (req, res) {
        ...
        consulta = "SELECT count(*) from dw.ft_modalidad mod left outer join dw.lk_seccional secc on secc.id_secc_seccional=mod.id_sec_seccional where  1=1";
        // Get a Postgres client from the connection pool
        pg.connect(connectionString.DAC, function (err, client, done) {
            var query_res = client.query("SET SEARCH_PATH TO ".concat(connectionString.DAC_schema));
    ...
    check_mapa: function (req, res) {
        ...
        consulta = "SELECT count(*) from dw.ft_modalidad mod left outer join dw.lk_seccional secc on secc.id_secc_seccional=mod.id_sec_seccional where  1=1 and lugar_lat is not null and lugar_long is not null";
        // Get a Postgres client from the connection pool
        pg.connect(connectionString.DAC, function (err, client, done) {
            var query_res = client.query("SET SEARCH_PATH TO ".concat(connectionString.DAC_schema));
    ...
    puntos_test: function (req, res) {
        var puntos = [];
        var leyenda = [];
        query_p = "SELECT  mod_colour as color_qgis2leaf ,case tentativa when 1 then 'circulo' else 'rombo' end as tipo ,id_actuacion as Actuacion ,id_fecha_hecho as FechaHecho ,hora_hecho ,lugar_hecho ,descripcion_hecho ,total_autores ,case uso_armas when 0 then 'NO' else 'SI' end as uso_armas ,lugar_lat ,lugar_long ,mod.id_mod_modalidad as modalidad from dw.ft_modalidad mod left outer join dw.lk_figura_modalidad fig on fig.id_mod_modalidad=mod.id_mod_modalidad left outer join dw.lk_seccional secc on secc.id_secc_seccional=mod.id_sec_seccional where  1=1 and lugar_lat is not null and lugar_long is not null and cod_bahra='0000000000' ";
        query_leyenda = "SELECT fig.desc_mod_figura||'-'||fig.desc_mod_modalidad as nombre ,mod_colour as color FROM dw.ft_modalidad mod left outer join dw.lk_figura_modalidad fig on fig.id_mod_modalidad=mod.id_mod_modalidad left outer join dw.lk_seccional secc on secc.id_secc_seccional=mod.id_sec_seccional where 1=1 and lugar_lat is not null and lugar_long is not null";
        // Get a Postgres client from the connection pool
        pg.connect(connectionString.DAC, function (err, client, done) {
            var query_res = client.query("SET SEARCH_PATH TO ".concat(connectionString.DAC_schema));
            ...
            if (typeof req.query.bar != 'undefined') {
                var j = 0;
                consulta = consulta.concat(" and cod_bahra in ( ");
                barrios = req.query.bar.split(",");
                for (i = 0; i < barrios.length; i++) {
