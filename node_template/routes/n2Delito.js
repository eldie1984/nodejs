...
var delito = {
    calendario: function (req, res) {
        ...
        pg.connect(connectionString.DAC, function (err, client, done) {
            ...
            query = "select min(a.cantidad) as minCant,MAX(a.cantidad)as maxCant,MIN(EXTRACT(YEAR FROM a.fecha)) as minFecha, (MAX(EXTRACT(YEAR FROM a.fecha))+1) as maxFecha from (select t.id_tie_fecha as fecha,id_tie_anio as anio, COUNT(*) as cantidad from ft_cargo mod left outer join lk_tiempo t on t.id_fecha=id_fecha_hecho left outer join lk_seccional secc on secc.id_secc_seccional=mod.id_sec_seccional where  1=1";

            if (typeof req.query.val != 'undefined' && req.query.val.length > 0) {
                query = query.concat(" and id_delito in (").concat(req.query.val).concat(")");
            }
            else {
                return res.status(500).send('ERRROOOOOOOR con la modalidad');
            }
    ...
    calendarioDatos: function (req, res) {
        ...
        pg.connect(connectionString.DAC, function (err, client, done) {
            var query_res = client.query("SET SEARCH_PATH TO ".concat(connectionString.DAC_schema));
            query = "select trim(substring( to_char(id_fecha_hecho, '99999999') from 1 for 5)||'-'||substring( to_char(id_fecha_hecho, '99999999') from 6 for 2)||'-'||substring( to_char(id_fecha_hecho, '99999999') from 8 for 2)) as id, COUNT(*) as value from ft_cargo mod left outer join lk_tiempo t on t.id_fecha=id_fecha_hecho left outer join lk_seccional secc on secc.id_secc_seccional=mod.id_sec_seccional where  1=1 ";
    ...
    datos: function (req, res) {
        ...
        pg.connect(connectionString.DAC, function (err, client, done) {
            ...
            {
                query = " FROM ft_cargo carg left outer join lk_seccional secc on secc.id_secc_seccional=carg.id_sec_seccional where 1=1  and id_fecha_hecho >= ".concat(req.query.fecha_d).concat(" and id_fecha_hecho <= ").concat(req.query.fecha_h)
            }
            else {
            ...
            if (typeof req.query.val != 'undefined' && req.query.val.length > 0) {
                ...
                return res.status(500).send('ERRROOOOOOOR con el modalidad');
            }
            ...
            query_res.on('error', function (row) {
                console.log(tot_query.concat(query));
                console.log(err);
                return res.status(500).send(err);
            });
            // After all data is returned, close connection and return results
    ...
    barrioMax: function (req, res) {
        ...
        pg.connect(connectionString.DAC, function (err, client, done) {
            ...
            if (typeof req.query.fecha_d != 'undefined' && typeof req.query.fecha_h != 'undefined') {
                query = "select cod_bahra as name,count(*) as densidad FROM ft_cargo carg left outer join lk_seccional secc on secc.id_secc_seccional=carg.id_sec_seccional where 1=1 and lugar_lat is not null and lugar_long is not null and id_fecha_hecho >= ".concat(req.query.fecha_d).concat(" and id_fecha_hecho <= ").concat(req.query.fecha_h)
            }
            else {
                return res.status(500).send('ERRROOOOOOOR con con las fechas desde y hasta');
            }
    ...
    barrioCloroDatos: function (req, res) {
        ...
        pg.connect(connectionString.DAC, function (err, client, done) {
            var query_res = client.query("SET SEARCH_PATH TO ".concat(connectionString.DAC_schema));
            query = "select cod_bahra as barrio,count(*) as cantidad FROM ft_cargo mod  left outer join lk_seccional secc on secc.id_secc_seccional=mod.id_sec_seccional where 1=1"
            if (typeof req.query.fecha_d != 'undefined' && typeof req.query.fecha_h != 'undefined') {
    ...
    tags: function (req, res) {
        ...
        pg.connect(connectionString.DAC, function (err, client, done) {
            var query_res = client.query("SET SEARCH_PATH TO ".concat(connectionString.DAC_schema));
            query = "select * from tagsdelitosn2(";
            palabras_consulta = '';
            if (typeof req.query.fecha_d != 'undefined' && typeof req.query.fecha_h != 'undefined') {
            ...
            if (typeof req.query.val != 'undefined') {
                consulta = " and id_delito in (".concat(req.query.val).concat(")");
            }
            else {
                return res.status(500).send('ERRROOOOOOOR con el delito');
            }
    ...
    total: function (req, res) {
        ...
        pg.connect(connectionString.DAC, function (err, client, done) {
            var query_res = client.query("SET SEARCH_PATH TO ".concat(connectionString.DAC_schema));
            query = "select * from ListardelitosN2(";
    ...
    hora: function (req, res) {
        ...
        pg.connect(connectionString.DAC, function (err, client, done) {
            var query_res = client.query("SET SEARCH_PATH TO ".concat(connectionString.DAC_schema));
            query = "select * from horariodelitosn2(";
    ...
    puntos: function (req, res) {
        var puntos = [];
        var leyenda = [];
        query_p = "SELECT  del_colour as color_qgis2leaf ,case tentativa when 1 then 'circulo' else 'rombo' end as tipo ,id_actuacion as Actuacion ,id_fecha_hecho as FechaHecho ,hora_hecho ,lugar_hecho ,descripcion_hecho ,total_autores ,case uso_armas when 0 then 'NO' else 'SI' end as uso_armas ,lugar_lat ,lugar_long ,mod.id_delito as modalidad from ft_cargo mod left outer join lk_delito fig on fig.id_del_delito=mod.id_delito left outer join lk_seccional secc on secc.id_secc_seccional=mod.id_sec_seccional where  1=1 and lugar_lat is not null and lugar_long is not null ";
        query_leyenda = "SELECT 'Ley '||id_del_ley|| ' Art. '||id_del_articulo||' Inc. '||coalesce(id_del_inciso,'')||' - '||desc_del_delito as nombre ,del_colour as color FROM ft_cargo mod left outer join lk_delito fig on fig.id_del_delito=mod.id_delito left outer join lk_seccional secc on secc.id_secc_seccional=mod.id_sec_seccional where 1=1 and lugar_lat is not null and lugar_long is not null";
        // Get a Postgres client from the connection pool
        pg.connect(connectionString.DAC, function (err, client, done) {
            var query_res = client.query("SET SEARCH_PATH TO ".concat(connectionString.DAC_schema));
            ...
            if (typeof req.query.val != 'undefined') {
                consulta = consulta.concat(" and mod.id_delito in (").concat(req.query.val).concat(")");
            }
            else {
                return res.status(500).send('ERRROOOOOOOR con el delito');
            }
    ...
    leyenda_ptos: function (req, res) {
        ...
        pg.connect(connectionString.DAC, function (err, client, done) {
            var query_res = client.query("SET SEARCH_PATH TO ".concat(connectionString.DAC_schema));
            if (typeof req.query.fecha_d != 'undefined' && typeof req.query.fecha_h != 'undefined') {
                query = "SELECT 'Ley '||id_del_ley|| ' Art. '||id_del_articulo||' Inc. '||coalesce(id_del_inciso,'')||' - '||desc_del_delito as nombre ,del_colour as color FROM ft_cargo mod left outer join lk_delito fig on fig.id_del_delito=mod.id_delito left outer join lk_seccional secc on secc.id_secc_seccional=mod.id_sec_seccional where 1=1 and lugar_lat is not null and lugar_long is not null and id_fecha_hecho >= ".concat(req.query.fecha_d).concat(" and id_fecha_hecho <= ").concat(req.query.fecha_h);
            }
            else {
                return res.status(500).send('ERRROOOOOOOR con con las fechas desde y hasta');
            }
    ...
    check: function (req, res) {
        ...
        consulta = "SELECT count(*) from ft_cargo mod left outer join lk_seccional secc on secc.id_secc_seccional=mod.id_sec_seccional where  1=1";
        // Get a Postgres client from the connection pool
        pg.connect(connectionString.DAC, function (err, client, done) {
            var query_res = client.query("SET SEARCH_PATH TO ".concat(connectionString.DAC_schema));
            if (typeof req.query.fecha_d != 'undefined' && typeof req.query.fecha_h != 'undefined') {
    ...
    check_mapa: function (req, res) {
        ...
        consulta = "SELECT count(*) from ft_cargo mod left outer join lk_seccional secc on secc.id_secc_seccional=mod.id_sec_seccional where  1=1 and lugar_lat is not null and lugar_long is not null ";
        // Get a Postgres client from the connection pool
        pg.connect(connectionString.DAC, function (err, client, done) {
            var query_res = client.query("SET SEARCH_PATH TO ".concat(connectionString.DAC_schema));
            if (typeof req.query.fecha_d != 'undefined' && typeof req.query.fecha_h != 'undefined') {
